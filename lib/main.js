...
function setTargetLanguage() {
  targetLanguage = $("#target-language").find(":selected").val();
}
...
let handleEventStreamMessage = function (messageJson) {
  ...
  if (results.length > 0) {
    if (results[0].Alternatives.length > 0) {
      ...
      //translate POC here
      // source language != target language -> polling 10s or 15s
    ...
    //   $("#transcript").val(transcription + transcript + "\n");
      ...
      if (!results[0].IsPartial) {
        ...
        // transcription += testAPI(transcript) + "\n"
        // testAPI(transcript)
...
$("#translate-button").click(function () {
  // translateText($('#text-to-translate').val())
  testAPI();
});

function testAPI(textToTranslate) {
  fetch(
    "https://js6ilai84h.execute-api.us-east-1.amazonaws.com/GenerateCanonicalRequestParams",
    {
      method: "POST",
      body: JSON.stringify({
        textToTranslate: textToTranslate,
        sourceLanguage: languageCode.substring(0, 2),
        targetLanguage: ($("#target-language").find(':selected').val()).substring(0, 2),
      }),
    }
  )
    .then((response) => {
      response.json().then((json) => {
        let jsonData = json[0]["body"][0]
        fetch(jsonData.endpoint, {
          method: "POST",
          body: jsonData.data,
          ...
          .then((response) => {
            response.json().then((json) => {
                ...
                $("#transcript").val(transcription + json.TranslatedText + "\n");
            });
          })
          .catch((error) => {
            console.log(error);
